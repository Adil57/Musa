document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadSliderImages() {
        ...
        try {
            ...
            // Agar koi item ya asset nahi hai, toh bahar nikal jao
            if (!data.items || data.items.length === 0 || !data.includes || !data.includes.Asset) {
                swiperWrapper.innerHTML = `<div class="swiper-slide"><p>No published content found. Please check Contentful.</p></div>`;
                return;
            }
            ...
            data.items.forEach(item => {
                const photoField = item.fields.photo;
                if (photoField) {
                    const photoId = photoField.sys.id;
            ...
            if (slidesHTML) {
                ...
                swiperWrapper.innerHTML = `<div class="swiper-slide"><p>Could not build slides. Check if entries are linked to assets properly.</p></div>`;
            }
    ...
    loadSliderImages();
    console.log("Musa's Portfolio Simplest Version Loaded!");
});
